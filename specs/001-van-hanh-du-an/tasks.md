...
- [X] T163 [P] Thêm `assignTask` và `setNextAction` vào `frontend/src/lib/api.ts` — hai lối vào `/v1/tasks/{id}/assign` và `/v1/tasks/{id}/next-action` chưa có bộ gọi nào ở giao diện. Hai đường còn lại đã có sẵn và phải dùng lại, không viết mới: **huỷ việc** đi qua `updateTaskStatus(id, 'cancelled', reason)` (máy chủ đã bắt buộc lý do — FR-030), **"tôi đã xử lý xong"** đi qua `resolveInboxItem(id)` (FR-061a, FR-070)
...
- [X] T165 Kiểm đường đóng mục sau khi bấm. **Lá thư đóng vì người chủ bấm, KHÔNG phải vì vòng quét** (FR-061b cấm vòng quét đụng vào mục hộp thư) — nên mỗi hành động ở T164 phải tự gọi đường đóng mục, và đường đó tính lại xem còn ai sắp chạm vào đầu việc không (FR-061c). Bốn đường phải đi thử đủ: giao lại → người mới được gọi dậy ngay lúc giao; đổi việc kế tiếp và **"tôi đã xử lý xong"** → không ai được gọi lúc bấm, vòng quét nhặt lại trong nhịp kế tiếp và bắt đầu **từ Mức 1**; huỷ việc → đầu việc rời khỏi tầm quét và mục thôi là câu hỏi. Chưa đường nào được đi thử
