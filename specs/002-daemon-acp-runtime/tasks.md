...
- [ ] T061 [US1] Viết `daemon/internal/execenv/tools.go` — bơm **bộ công cụ gọi ngược theo từng lượt chạy** qua cơ chế nạp công cụ sẵn có của từng CLI, mang **token của lượt chạy**. Cùng luật với kỹ năng: không ghi vào cấu hình dùng chung (FR-013, FR-013a). **Chặn** (ghi 2026-08-26): chưa có thứ để trỏ tới. FR-013a đòi bơm qua **cơ chế nạp công cụ sẵn có của từng CLI**, tức là phải khai một tool server vào cấu hình theo lượt chạy — mà T125 đã xoá hẳn package `mcp/`, còn lối `/agent/*` duy nhất còn lại thì xác thực bằng **token sống lâu của agent**, thứ FR-014a không chừa chỗ cho. Vậy trước T061 phải có một mặt gọi ngược nhận **token của lượt chạy**: T064 (đúc và trao token) rồi T039d (gỡ token sống lâu). Viết bây giờ là khai một địa chỉ không ai trả lời, hoặc bơm chính cái token vừa bị cấm
