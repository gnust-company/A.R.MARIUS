...
# Bỏ dự án đi là vứt cả cái đã đông cứng, không phải thao tác bên trong nó. Đóng băng nội
# dung mà vẫn không xoá được thì người chủ mắc kẹt vĩnh viễn với một dự án bỏ đi.
_DISPOSES_OF_THE_PROJECT = ("DELETE", "/v1/projects/{project_id}")
...
def test_no_write_route_touching_a_project_escapes_the_guard() -> None:
    escaped: list[str] = []
    for route in app.routes:
        methods = getattr(route, "methods", None) or set()
        path = getattr(route, "path", "")
        if "{project_id}" not in path and "{task_id}" not in path:
            continue
        writes = methods - _READ_ONLY
        if not writes:
            continue
        if (sorted(writes)[0], path) == _DISPOSES_OF_THE_PROJECT:
            continue
        if not _is_guarded(getattr(route, "dependant", None)):
            escaped.append(f"{sorted(writes)[0]} {path}")

    assert not escaped, (
        "mấy lối ghi này chạm vào dự án mà không qua chỗ chặn đóng băng: " + ", ".join(escaped)
    )
