...
này cho giao diện: nó dựng `RosterRoleView`/`SeatView` với `role.title`, `role.description` thật. Các
prompt cấp dự án nay **cũng** tra chuỗi này (đã sửa ở #87 — xem §3), không còn đọc nhầm `Marius.role` (rỗng).

> **[ĐÚNG-NHƯ-CODE] mô tả vai trò một trường:** `RoleSpec`/`Role` nay chỉ mang **một** trường
> `description` (trường cũ `responsibilities` đã gỡ ở #93); `get_roster` trả `description`. Xem
...
> Ghi chú: "mô tả vai trò" nay là **một** trường `Role.description` duy nhất — trường cũ
> `responsibilities` đã gỡ ở #93 (xem [01-domain.md](01-domain.md) §3.2).
